...
let ownerPrivkey = process.env.OWNER_PRIVKEY;
let owner: Keypair;
// payer for transactions
let payerPrivkey = process.env.PAYER_PRIVKEY;
let payer: Keypair;
// token account pool
let tokenSwapAccountPrivkey = process.env.TOKEN_SWAP_ACCOUNT_PRIVKEY;
...
export async function createTokenSwap(
  ...
  payer = payerPrivkey ? Keypair.fromSecretKey(bs58.decode(payerPrivkey)) : await newAccountWithLamports(connection, LAMPORTS_PER_SOL);;
  owner = ownerPrivkey ? Keypair.fromSecretKey(bs58.decode(ownerPrivkey)) : await newAccountWithLamports(connection, LAMPORTS_PER_SOL);;
  tokenSwapAccount = tokenSwapAccountPrivkey ? Keypair.fromSecretKey(bs58.decode(tokenSwapAccountPrivkey)) : Keypair.generate();
